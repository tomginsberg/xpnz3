...
async function makeTransactionsTable() {
  ...
  await db.schema.raw(
    "ALTER TABLE `transactions` ADD COLUMN `ledger` varchar(255) collate nocase REFERENCES `ledgers` (`name`)"
  )
}
...
async function makeMembersTable() {
  ...
  await db.schema.raw(
    "ALTER TABLE `members` ADD COLUMN `ledger` varchar(255) collate nocase REFERENCES `ledgers` (`name`)"
  )
...
async function importTransactionsFromJsonFile(filename, ledgername) {
  ...
  for (var tx of txs) {
    var transactionMemberPairs = reduceToObject(
      membersAndIds,
      ({ name }) => name,
      ({ id }) => ({ member_id: id, amount: 0, weight: 0 })
    )

    tx.for.members.forEach((name, index) => (transactionMemberPairs[name].weight = tx.for.split_weights[index]))
    tx.by.members.forEach(
      (name, index) => (transactionMemberPairs[name].amount = Math.floor(Math.abs(tx.by.split_values[index]) * 100))
    ) // convert to cents
    ...
    transactionMemberPairs = transactionMemberPairs.map((pair) => ({
      ...pair,
      weight: pair.weight < 0 ? -pair.weight : pair.weight
    }))
