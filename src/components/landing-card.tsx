      ...
      >
        <div className="p-4">
          <div className="flex flex-wrap mb-2 justify-between">
            <h2 className="break-normal pr-3 text-ellipsis overflow-hidden text-balance text-lg font-extrabold tracking-tight">
              {expense.name}
            </h2>
            ...
            >
              ...
              <div className="p-4 text-sm">
                <p>
                  ...
                  {expense.paidBy.map(({ member, amount }) => `${member} ($${amount.toFixed(2)})`).join(', ')}
                </p>
                <p className="mt-1">
                  <span className="font-semibold">Split between: </span>
                  {expense.splitBetween
                    .map(({ member, normalizedWeight }) => `${member} ($${normalizedWeight.toFixed(2)})`)
                    .join(', ')}
                </p>
              </div>
            </motion.div>
